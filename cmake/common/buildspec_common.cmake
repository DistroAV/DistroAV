...
function(_check_deps_version version)
    ...
    foreach(path IN LISTS CMAKE_PREFIX_PATH)
        if(EXISTS "${path}/share/obs-deps/VERSION")
            if(
                dependency STREQUAL qt6
                AND NOT EXISTS "${path}/lib/cmake/Qt6/Qt6Config.cmake"
            )
                set(found FALSE)
                continue()
            endif()
...
function(_setup_obs_studio)
    ...
    execute_process(
        COMMAND
            ...
            -DOBS_CMAKE_VERSION:STRING=3.0.0 -DENABLE_PLUGINS:BOOL=OFF
    ...
    execute_process(
        COMMAND
            "${CMAKE_COMMAND}" --build build_${arch} --target obs-frontend-api
    ...
    execute_process(
        COMMAND
            "${CMAKE_COMMAND}" --install build_${arch} --component Development
