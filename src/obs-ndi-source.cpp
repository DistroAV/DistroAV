...
obs_properties_t *ndi_source_getproperties(void *data)
{
    ...
    obs_property_t *source_list = obs_properties_add_list(
        props, PROP_SOURCE, obs_module_text("NDIPlugin.SourceProps.SourceName"),
    ...
    obs_property_t *bw_modes = obs_properties_add_list(
        ...
        obs_module_text("NDIPlugin.SourceProps.Bandwidth"), OBS_COMBO_TYPE_LIST,
        OBS_COMBO_FORMAT_INT);

    obs_property_list_add_int(
        bw_modes, obs_module_text("NDIPlugin.BWMode.Highest"), PROP_BW_HIGHEST);
    obs_property_list_add_int(
        bw_modes, obs_module_text("NDIPlugin.BWMode.Lowest"), PROP_BW_LOWEST);
    ...
    obs_property_list_add_int(
        sync_modes, obs_module_text("NDIPlugin.SyncMode.NDISourceTimecode"),
    ...
    obs_property_list_add_int(
        yuv_ranges, obs_module_text("NDIPlugin.SourceProps.ColorRange.Partial"),
        PROP_YUV_RANGE_PARTIAL);
    obs_property_list_add_int(
        yuv_ranges, obs_module_text("NDIPlugin.SourceProps.ColorRange.Full"),
        PROP_YUV_RANGE_FULL);
    ...
    obs_property_t *latency_modes = obs_properties_add_list(
        props, PROP_LATENCY, obs_module_text("NDIPlugin.SourceProps.Latency"),
        OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);

    obs_property_list_add_int(
        latency_modes, obs_module_text("NDIPlugin.SourceProps.Latency.Normal"),
        PROP_LATENCY_NORMAL);
    obs_property_list_add_int(
        latency_modes, obs_module_text("NDIPlugin.SourceProps.Latency.Low"),
        PROP_LATENCY_LOW);
    ...
    obs_properties_add_button(
        ...
        [](obs_properties_t *pps, obs_property_t *prop, void *private_data) {
...
void *ndi_source_poll_audio_video(void *data)
{
    ...
    while (s->running) {
        ...
        frame_received = ndiLib->recv_capture_v3(s->ndi_receiver, &video_frame,
        ...
        if (frame_received == NDIlib_frame_type_audio) {
            ...
            if (s->audio_enabled) {
                const int channelCount =
                    audio_frame.no_channels > 8 ? 8 : audio_frame.no_channels;
        ...
        if (frame_received == NDIlib_frame_type_video) {
            ...
            default:
                blog(LOG_INFO, "warning: unsupported video pixel format: %d",
                     video_frame.FourCC);
                break;
            }
