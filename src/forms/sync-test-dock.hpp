...
private:
	...
	// Throttle ms display updates to once per second (timestamps still update every frame)
	uint64_t last_ms_display_update_ns = 0;
...
private:
	...
	void on_obs_frame_output(int64_t render_wall_clock_ns, int64_t source_frame_ts);
