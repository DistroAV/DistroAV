...
void RemoteTextThread::run()
{
	...
	if (curl) {
		...
		if (!postData.empty()) {
			curl_easy_setopt(session, CURLOPT_POSTFIELDS, postData.c_str());
		}
		...
		if (curlCode != CURLE_OK) {
			errorData = QString("{ \"curlCode\":{ \"code\":%1, \"text\":\"%2\" }, \"error\":\"%3\" }")
					    .arg(curlCode)
...
bool GetRemoteFile(const char *url, std::string &str, std::string &error, long *responseCode, const char *contentType,
		   std::string request_type, const char *postData, std::vector<std::string> headers,
