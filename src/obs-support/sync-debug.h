...
//#define SYNC_DEBUG 1
#ifdef SYNC_DEBUG
#define OBS_SYNC_DEBUG_LOG_VIDEO_TIME(message, source_ndi_name, timestamp, data)                              \
    if (data) obs_sync_debug_log_video_time(message, source_ndi_name, timestamp, data)
#else
#define OBS_SYNC_DEBUG_LOG_VIDEO_TIME(message, source_ndi_name, timestamp, data)                                \
	do {} while (0)
#endif
#ifdef SYNC_DEBUG
#define OBS_SYNC_DEBUG_LOG_AUDIO_TIME(message, source_ndi_name, timestamp, data, no_samples, sample_rate)       \
	if (data) obs_sync_debug_log_audio_time(message, source_ndi_name, timestamp, \
