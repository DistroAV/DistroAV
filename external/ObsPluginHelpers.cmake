...
function(install_external_plugin_data_internal target source_dir target_dir)
  ...
  add_custom_command(
    ...
    COMMAND "${CMAKE_COMMAND}" -E copy_directory "${CMAKE_CURRENT_SOURCE_DIR}/${source_dir}"
            "${EXTERNAL_PLUGIN_OUTPUT_DIR}/$<CONFIGURATION>/${target}/${target_dir}"
    VERBATIM)
endfunction()

# Installs data 'target' is the destination target project being installed to 'data_loc' specifies
# the directory of the data
function(install_external_plugin_data target data_loc)
  install_external_plugin_data_internal(${target} ${data_loc} "data")
endfunction()

# Installs data in an architecture-specific data directory on windows/linux (data/32bit or
# data/64bit).  Does not apply for mac. 'target' is the destination target project being installed
# to 'data_loc' specifies the directory of the data being installed
...
# Installs data in the target's bin directory 'target' is the destination target project being
# installed to 'data_loc' specifies the directory of the data being installed
...
# Installs an additional binary to a target 'target' is the destination target project being
# installed to 'additional_target' specifies the additional binary
function(install_external_plugin_additional target additional_target)
  ...
  set_target_properties(${additional_target} PROPERTIES PREFIX "")

  install(
    TARGETS ${additional_target}
  ...
  add_custom_command(
    ...
    POST_BUILD
    COMMAND
      "${CMAKE_COMMAND}" -E copy "$<TARGET_FILE:${additional_target}>"
      "${EXTERNAL_PLUGIN_OUTPUT_DIR}/$<CONFIGURATION>/${target}/bin/${_bit_suffix}$<TARGET_FILE_NAME:${additional_target}>"
    VERBATIM)
endfunction()

# Installs the binary of the target 'target' is the target project being installed
function(install_external_plugin target)
  install_external_plugin_additional(${target} ${target})
endfunction()

# Installs the binary and data of the target 'target' is the destination target project being
# installed to
...
# Installs an additional binary to the data of a target 'target' is the destination target project
# being installed to 'additional_target' specifies the additional binary
...
# Installs an additional binary in an architecture-specific data directory on windows/linux
# (data/32bit or data/64bit).  Does not apply for mac. 'target' is the destination target project
# being installed to 'additional_target' specifies the additional binary
...
# Installs an additional file in an architecture-specific data directory on windows/linux
# (data/32bit or data/64bit).  Does not apply for mac. 'target' is the destination target project
# being installed to 'additional_target' specifies the additional binary
function(install_external_plugin_data_file_to_arch_data target additional_target file_target)
  ...
  add_custom_command(
    ...
    POST_BUILD
    COMMAND
      "${CMAKE_COMMAND}" -E copy "${file_target}"
